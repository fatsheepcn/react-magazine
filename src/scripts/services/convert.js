...
function makeHtml(data) {
    var picList = _uniq(data.nimg, function(img) {
            return img.id;
        }),
...
function detailConverter(data) {
    ...
    return {
        ...
        content: (_findIndex(gallaryTypes, data.type, function(gt) {
            return gt == data.type;
        }) !== -1 ? makeGallaryContent(data) : makeHtml(data)),
        lnk: data.lnk
    };
}

function listConverter(data) {
